...
import { Form, Input, Typography, Select, Button, Avatar, Modal,TreeSelect } from 'antd';
import { Link } from 'react-router-dom';
import "./EditOrganisationViewComponent.scss";
...
export const EditOrganisationViewComponent = () => {
  ...
  const countDown = () => {
    const modal = Modal.success({
      ...
      content: `New Password has been sent to ${parsedUpdatedCardData[0].email}`,
    });
  ...
  let updatedCardData = localStorage.getItem("item");

  let parsedUpdatedCardData = JSON.parse(updatedCardData);

  let updatedData = localStorage.getItem("organization");
    let parsedUpdatedData = JSON.parse(updatedData);

  const reSettingLocalStorageData = (cardData) => {
    
    parsedUpdatedData[parsedUpdatedCardData[0].getIndex].orgName  = cardData.orgName
    parsedUpdatedData[parsedUpdatedCardData[0].getIndex].email  = cardData.email
    parsedUpdatedData[parsedUpdatedCardData[0].getIndex].orgDomain  = cardData.orgDomain
    parsedUpdatedData[parsedUpdatedCardData[0].getIndex].service  = cardData.service
    parsedUpdatedData[parsedUpdatedCardData[0].getIndex].address  = cardData.address
    parsedUpdatedData[parsedUpdatedCardData[0].getIndex].state  = cardData.state
    parsedUpdatedData[parsedUpdatedCardData[0].getIndex].phoneNumber  = cardData.phoneNumber
    parsedUpdatedData[parsedUpdatedCardData[0].getIndex].admina  = cardData.admina
    parsedUpdatedData[parsedUpdatedCardData[0].getIndex].city  = cardData.city
 
     
    localStorage.setItem("organization",JSON.stringify(parsedUpdatedData))
        ...
        >
          <div className="add-organisation-form-image-container">
            <Form.Item valuePropName="fileList">
              <Avatar
                src={parsedUpdatedCardData[0].logo}
                  ...
                  <Form>
                    ...
                    <Form.Item>
                      ...
                      <Button type="primary" disabled={!disablebutton} onClick={countDown}>Reset</Button>
                        {/* <Modal
                          open={isResetSuccessfull}
                          footer = {null}
                          onCancel={closeSuccessfullPage}
                          closable={true}
                        >
                          <div className="password-changed-container">
                            <div className="tick-icon-container">
                                <img src="./logos/tick.png" alt="tick icon"/>
                            </div>
                            <h1>Password Changed</h1>
                            <p>New password link has been sent to the {Item.orgemail}</p>
                          </div>
                        </Modal> */}
                      </div>
                    </Form.Item>
          ...
          <div className="add-organisation-form-fillup-container">
            ...
            >
              <Input
                ...
                style={{ width: "100%" }}
                defaultValue = {parsedUpdatedCardData[0].orgName}
              />
            </Form.Item>
            <Form.Item
            ...
            >
              <Input
                ...
                defaultValue={parsedUpdatedCardData[0].phoneNumber}
              />
            </Form.Item>
            <Form.Item
              label="Email :"
            ...
            >
              <Input
                ...
                defaultValue={parsedUpdatedCardData[0].email}
              />
            </Form.Item>
            <Form.Item
              label="City :"
            ...
            >
              <Input
                ...
                defaultValue={parsedUpdatedCardData[0].city}
              />
            </Form.Item>
            <Form.Item
              label="State :"
            ...
            <Form.Item
              ...
              rules={[{ required: true, message: "Please enter state" }]}
            >
              <Select defaultValue={parsedUpdatedCardData[0].orgDomain} style={{ width: "100%" }}>
            ...
            >
              <Input
                ...
                defaultValue={parsedUpdatedCardData[0].address}
              />
            </Form.Item>
